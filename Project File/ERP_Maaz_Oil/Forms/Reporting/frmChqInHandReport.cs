...
{
    public partial class frmChqInHandReport : Form
    {
        public frmChqInHandReport()
        {
            InitializeComponent();
        }
        ...
        {
            string query = @"SELECT CAST(DATE as date) as [DATE],E.COA_NAME AS [REC_FROM],
                    G.AMOUNT as[AMOUNT],
                    ...
                    WHERE ENTRY_OF='RECEIPT VOUCHER' AND G.STATUS = 1
                    AND DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'";

            if (cmbSalesPerson.SelectedIndex > 0)
                query = @"SELECT CAST(DATE as date) as [DATE],E.COA_NAME AS [REC_FROM],
                        G.AMOUNT as[AMOUNT],
                        ISNULL(G.CHQ_NO,'-') as [CHQ_NO],ISNULL(G.BANK_NAME,'-') as [BANK],CONVERT(date,G.CHQ_DATE) as [CHQ_DATE]
            ...
            query+= " ORDER BY E.COA_NAME,G.BANK_NAME ";
        ...
        public void show_report()
        {
            ...
            if (cmbSalesPerson.SelectedIndex > 0)
                classHelper.rpt.headingTextChange = "SALES PERSON WISE CHQ IN HAND REPORT";
            else if (chckChqDate.Checked)
                classHelper.rpt.headingTextChange = "CHEQUE DATE WISE CHQ IN HAND REPORT";
            else if (cmbCustomer.SelectedIndex > 0)
                classHelper.rpt.headingTextChange = "CUSTOMER WISE CHQ IN HAND REPORT";
            
                
            
            classHelper.rpt.GenerateReport("CHQINHAND", classHelper.nds);
