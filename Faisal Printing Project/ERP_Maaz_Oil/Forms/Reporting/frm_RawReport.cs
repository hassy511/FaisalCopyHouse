    ...
    public partial class frm_RawReport : Form
    {
        ...
        public void GenerateReport()
        {
            ...
            ISNULL(
	            ...
	            WHERE X.[DATE] <= '" + Classes.Helper.ConvertDatetime(dtpFrom.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59)) + @"' AND Y.MATERIAL_ID = D.MATERIAL_ID)
            ,0) +
	        ISNULL((
		        SELECT SUM(X.QTY) 
				FROM INVENTORY_ADJUSTMENTS_RAW X
				INNER JOIN MATERIALS Z ON X.MATERIAL_ID = Z.MATERIAL_ID
				WHERE X.[DATE] <= '" + Classes.Helper.ConvertDatetime(dtpFrom.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59)) + @"' AND X.MATERIAL_ID = D.MATERIAL_ID
				AND X.ADD_LESS = 'A'
	        ),0) AS [IN],
            ISNULL((
		    SELECT SUM(O.QTY) FROM (
			        ...
			        WHERE X.[DATE] <= '" + Classes.Helper.ConvertDatetime(dtpFrom.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59)) + @"' AND D.MATERIAL_ID = V.MATERIAL_ID
			        ...
			        WHERE X.[DATE] <= '" + Classes.Helper.ConvertDatetime(dtpFrom.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59)) + @"' AND D.MATERIAL_ID = V.MATERIAL_ID  AND Y.ITEM_TYPE = 'R'
			...
			ISNULL(
	            ...
	            INNER JOIN PURCHASE_RETURN_DETAIL Y ON X.ID = Y.MASTER_ID
	            INNER JOIN MATERIALS V ON V.MATERIAL_ID = Y.MATERIAL_ID
	            WHERE X.[DATE] <= '" + Classes.Helper.ConvertDatetime(dtpFrom.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59)) + @"' AND Y.MATERIAL_ID = D.MATERIAL_ID)
            ,0) +
	        ISNULL((
