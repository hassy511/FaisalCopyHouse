...
{
    public partial class frm_SalesInvoiceReportProvinceSalePerson : Form
    {
        Classes.Helper classHelper = new Classes.Helper();
        public frm_SalesInvoiceReportProvinceSalePerson()
        {
            InitializeComponent();
        }
        ...
        private void ShowReport()
        {
            ...
            insert into #temp1
	            select at.[SALES PERSON],at.PROVINCE,sum(at.[CANOLA TOTAL WEIGHT]),
	            sum(at.[CANOLA AMOUNT]),avg(at.[CANOLA AVG RATE])
	            from (                
            ...
            UNION ALL
	            ...
	            WHERE E.MATERIAL_ID = '5005' 
	            AND A.DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
	            GROUP BY F.NAME,J.REGION_NAME) at
	            group by at.[SALES PERSON],at.PROVINCE
            UNION ALL
	            select at.[SALES PERSON],at.PROVINCE,sum(at.[CANOLA TOTAL WEIGHT]),
	            sum(at.[CANOLA AMOUNT]),avg(at.[CANOLA AVG RATE])
	            from (                
            ...
            insert into #temp2
	            select bt.[SALES PERSON],bt.PROVINCE,sum(bt.[OLIEN TOTAL WEIGHT]),
	            sum(bt.[OLIEN AMOUNT]),avg(bt.[OLIEN AVG RATE])
	            from (
	            SELECT F.NAME AS[SALES PERSON], J.REGION_NAME AS[PROVINCE],
            ...
            UNION ALL
	            ...
	            WHERE E.MATERIAL_ID = '5003' 
	            AND A.DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
	            GROUP BY F.NAME,J.REGION_NAME) bt
	            group by bt.[SALES PERSON],bt.PROVINCE
            UNION ALL
	            select bt.[SALES PERSON],bt.PROVINCE,sum(bt.[OLIEN TOTAL WEIGHT]),
	            sum(bt.[OLIEN AMOUNT]),avg(bt.[OLIEN AVG RATE])
	            from (
            ...
            classHelper.query = @"create table #temp1(
                ...
                insert into #temp1
select at.PROVINCE,sum(at.[CANOLA TOTAL WEIGHT]),sum(at.[CANOLA AMOUNT]),
				avg(at.[CANOLA AVG RATE])
				from (                
				SELECT J.NAME AS [PROVINCE],
                SUM((C.WEIGHT)) AS [CANOLA TOTAL WEIGHT],SUM((C.QTY) * C.RATE) AS [CANOLA AMOUNT],
                SUM((C.QTY) * C.RATE) / SUM((C.WEIGHT)) AS [CANOLA AVG RATE]
                FROM SALES A
                INNER JOIN SALES_PROGRAM_MASTER B ON A.SOP_ID = B.SPM_ID
                ...
                INNER JOIN CUSTOMER_PROFILE H ON H.COA_ID = G.COA_ID
                --INNER JOIN CITY I ON H.CITY_ID = I.CITY_ID
                INNER JOIN SALES_PERSONS J ON H.SALE_PER_ID = J.SALES_PER_ID
                WHERE E.MATERIAL_ID = '5005'
                AND A.DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
                GROUP BY J.NAME
                UNION ALL
                ...
                WHERE E.MATERIAL_ID = '5005'
                AND A.DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
                GROUP BY J.NAME) as at
				group by at.PROVINCE
				UNION ALL
				select at.PROVINCE,sum(at.[CANOLA TOTAL WEIGHT]),sum(at.[CANOLA AMOUNT]),
				avg(at.[CANOLA AVG RATE])
				from (                
            ...
            UNION ALL
                ...
                insert into #temp2
                select bt.PROVINCE,sum(bt.[OLIEN TOTAL WEIGHT]),sum(bt.[OLIEN AMOUNT]),
				avg(bt.[OLIEN AVG RATE])
				from (
				SELECT F.NAME AS[PROVINCE],
                SUM((C.WEIGHT)) AS[OLIEN TOTAL WEIGHT], SUM((C.QTY) * C.RATE) AS[OLIEN AMOUNT],
                ...
                WHERE E.MATERIAL_ID = '5003'
                AND A.DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
                GROUP BY J.NAME) as bt
				group by bt.PROVINCE
				UNION ALL
				select bt.PROVINCE,sum(bt.[OLIEN TOTAL WEIGHT]),sum(bt.[OLIEN AMOUNT]),
				avg(bt.[OLIEN AVG RATE])
				from (
            ...
            if (hasRows == 'Y')
            {
                ...
                classHelper.rpt.GenerateReport("ProvinceWiseReportSalePerson", classHelper.mds);
                classHelper.rpt.ShowDialog();

                classHelper.rpt = new frmReports();
                classHelper.rpt.GenerateReport("ProvinceWiseReportSummarySalePerson", classHelper.mds);
                classHelper.rpt.ShowDialog();
            }
        }
