...
{
    public partial class frm_SalesInvoiceReportProvince : Form
    {
        Classes.Helper classHelper = new Classes.Helper();
        public frm_SalesInvoiceReportProvince()
        {
            InitializeComponent();
        }
        ...
        private void ShowReport()
        {
            ...
            insert into #temp1
	            SELECT F.NAME AS [SALES PERSON],J.REGION_NAME AS [PROVINCE],G.COA_NAME AS [CUSTOMER NAME],
	            ...
	            AND F.SALES_PER_ID = '" + cmbSalePerson.SelectedValue.ToString() + @"'
	            GROUP BY F.NAME,J.REGION_NAME,G.COA_NAME
            UNION ALL
	            SELECT F.NAME AS [SALES PERSON],J.REGION_NAME AS [PROVINCE],G.COA_NAME AS [CUSTOMER NAME],
	            SUM((C.WEIGHT)) AS [CANOLA TOTAL WEIGHT],SUM((C.QTY) * C.RATE) AS [CANOLA AMOUNT],
	            SUM((C.QTY) * C.RATE) / SUM((C.WEIGHT)) AS [CANOLA AVG RATE]
            ...
            select x.sales_person,x.province,x.customerName,
            ...
            select *from #temp1
            union all
            SELECT sales_person, province, customerName, null, null, null FROM #temp2 WHERE
            customerName not in (select customerName from #temp1)
            ) x on x.customerName = a.customerName
