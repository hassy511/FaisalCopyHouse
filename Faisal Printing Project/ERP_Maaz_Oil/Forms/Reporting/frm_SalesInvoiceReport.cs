...
namespace ERP_Maaz_Oil.Forms.Reporting
{
    public partial class frm_SalesInvoiceReport : Form
    {
        ...
        {
            classHelper.LoadRawMaterials(cmbRawMaterial);
        }
        ...
        private void ShowReport()
        {
            ...
            if (chkProduct.Checked == false && chkRaw.Checked == false) {
                classHelper.query = @"	SELECT A.SALE_MASTER_ID,A.DATE,A.INVOICE_NO,DATEADD(DAY,A.CREDIT_DAYS,A.DATE) AS [DUE DATE],
	            D.COA_NAME AS [ACCOUNT NAME],C.PRODUCT_NAME,B.QTY,B.RATE,B.QTY*B.RATE AS [TOTAL]
	            FROM SALE_MASTER A
	            INNER JOIN SALE_DETAIL B ON A.SALE_MASTER_ID = B.SALE_MASTER_ID
	            INNER JOIN PRODUCT_MASTER C ON B.ITEM_ID = C.PM_ID
	            INNER JOIN COA D ON A.CUSTOMER_ID = D.COA_ID
	            WHERE D.STAT = '0' AND B.ITEM_TYPE = 'P' AND [DATE] BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
	            UNION ALL
	            SELECT A.SALE_MASTER_ID,A.DATE,A.INVOICE_NO,DATEADD(DAY,A.CREDIT_DAYS,A.DATE) AS [DUE DATE],
	            D.COA_NAME AS [ACCOUNT NAME],C.MATERIAL_NAME,B.QTY,B.RATE,B.QTY*B.RATE AS [TOTAL]
	            FROM SALE_MASTER A
	            INNER JOIN SALE_DETAIL B ON A.SALE_MASTER_ID = B.SALE_MASTER_ID
	            INNER JOIN MATERIALS C ON B.ITEM_ID = C.MATERIAL_ID
	            INNER JOIN COA D ON A.CUSTOMER_ID = D.COA_ID
	            WHERE D.STAT = '0' AND B.ITEM_TYPE = 'R' AND [DATE] BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'";
            }
            else if (chkProduct.Checked == true && cmbProduct.SelectedIndex == 0)
            {
                classHelper.query = @"	SELECT A.SALE_MASTER_ID,A.DATE,A.INVOICE_NO,DATEADD(DAY,A.CREDIT_DAYS,A.DATE) AS [DUE DATE],
	            D.COA_NAME AS [ACCOUNT NAME],C.PRODUCT_NAME,B.QTY,B.RATE,B.QTY*B.RATE AS [TOTAL]
	            FROM SALE_MASTER A
            ...
            else if (chkProduct.Checked == true && cmbProduct.SelectedIndex > 0)
            {
	            ...
	            INNER JOIN COA D ON A.CUSTOMER_ID = D.COA_ID
	            WHERE D.STAT = '0' AND B.ITEM_TYPE = 'P' AND [DATE] BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
                AND C.PM_ID = '" + cmbProduct.SelectedValue.ToString()+"'";
            }
            else if (chkRaw.Checked == true && cmbRawMaterial.SelectedIndex == 0)
            {
                classHelper.query = @"	SELECT A.SALE_MASTER_ID,A.DATE,A.INVOICE_NO,DATEADD(DAY,A.CREDIT_DAYS,A.DATE) AS [DUE DATE],
	            D.COA_NAME AS [ACCOUNT NAME],C.MATERIAL_NAME AS [PRODUCT_NAME],B.QTY,B.RATE,B.QTY*B.RATE AS [TOTAL]
	            FROM SALE_MASTER A
	            INNER JOIN SALE_DETAIL B ON A.SALE_MASTER_ID = B.SALE_MASTER_ID
	            INNER JOIN MATERIALS C ON B.ITEM_ID = C.MATERIAL_ID
	            INNER JOIN COA D ON A.CUSTOMER_ID = D.COA_ID
            ...
            else if (chkRaw.Checked == true && cmbRawMaterial.SelectedIndex > 0)
            {
	            ...
	            INNER JOIN COA D ON A.CUSTOMER_ID = D.COA_ID
	            WHERE D.STAT = '0' AND B.ITEM_TYPE = 'R' AND [DATE] BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
                AND C.MATERIAL_ID = '"+cmbRawMaterial.SelectedValue.ToString()+"'";
            }
            ...
            try
            {
                ...
                if (classHelper.dr.HasRows == true)
                {
                    ...
                    while (classHelper.dr.Read())
                    {
                        ...
                        classHelper.dataR["customer"] = classHelper.dr["ACCOUNT NAME"].ToString();
            ...
            //classHelper.query = @"SELECT D.COA_NAME,sa.INVOICE_NO, SUM(C.WEIGHT) AS [TOTAL WEIGHT],SA.[DATE],
            //(
            ...
            //(
	           ...
	           // INNER JOIN PRODUCT_MASTER E ON C.PRODUCT_ID = E.PM_ID
	           // INNER JOIN PRODUCT_DETAILS G ON E.PM_ID = G.PM_ID
	           // WHERE G.MATERIAL_ID = '5003' AND A.S_ID = SA.S_ID 
            ...
            //(
	           ...
	           // INNER JOIN COA D ON B.CUSTOMER_ID = D.COA_ID
	           // INNER JOIN MATERIALS E ON C.PRODUCT_ID = E.MATERIAL_ID
	           // WHERE E.MATERIAL_ID = '5003' AND A.S_ID = SA.S_ID 
	           // AND A.DATE BETWEEN '" + dtp_FROM.Value.Date + "' AND '" + dtp_TO.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59) + @"'
            //) 
            //AS [OLIEN AMOUNT],
            //(
            ...
            //GROUP BY D.COA_NAME,SA.S_ID,SA.INVOICE_NO,SA.[DATE]
            //UNION ALL
            ...
            //            classHelper.dataR["amout"] = Convert.ToDecimal(classHelper.dr["OLIEN AMOUNT"].ToString()) + Convert.ToDecimal(classHelper.dr["CANOLA AMOUNT"].ToString());
            ...
            if (hasRows == 'Y')
            {
                ...
                classHelper.rpt.GenerateReport("SalesInvoiceReport", classHelper.mds);
                classHelper.rpt.ShowDialog();

                //classHelper.rpt = new frmReports();
                //classHelper.rpt.GenerateReport("SalesInvoiceSummaryReport", classHelper.mds);
                //classHelper.rpt.ShowDialog();
            }
        }
