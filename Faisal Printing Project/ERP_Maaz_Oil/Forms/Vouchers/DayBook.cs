...
namespace ERP_Maaz_Oil.Vouchers
{
    public class DayBook
    {
        ...
        public void loadAllReceiptAccounts(ComboBox cmb)
        {
            db.LoadComboData(cmb, 
                @" SELECT '0' AS[ID],'--SELECT ACCOUNT--' AS [NAME] 
                UNION ALL
                SELECT COA_ID AS [ID],COA_NAME AS [NAME] FROM COA WHERE STAT=0 
                UNION ALL 
                SELECT COA_ID AS [ID],COA_NAME AS [NAME] FROM COA WHERE STAT=1 
                AND COA_ID IN (SELECT CREDIT_AC FROM DAY_BOOK)
                ORDER BY NAME"
            );
        }

        public void loadAllPaymentAccounts(ComboBox cmb)
        {
            db.LoadComboData(cmb,
                @" SELECT '0' AS[ID],'--SELECT ACCOUNT--' AS [NAME] 
                UNION ALL
                ...
                UNION ALL 
                SELECT COA_ID AS [ID],COA_NAME AS [NAME] FROM COA WHERE STAT=1 
                AND COA_ID IN (SELECT DEBIT_AC FROM DAY_BOOK)
                ORDER BY NAME"
            );
        }
