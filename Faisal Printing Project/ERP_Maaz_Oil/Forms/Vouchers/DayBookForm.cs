...
namespace ERP_Maaz_Oil.Vouchers
{
    public partial class DayBookForm : Form
    {
        ...
        private void PaymentTotalCalculation()
        {
            try
            {
                ...
                if (!txtPayCashAmount.Text.Equals(""))
                {
                    cashAmount = Convert.ToDecimal(txtPayCashAmount.Text);
                }

                if (!txtTotalGrdPayment.Text.Equals(""))
                {
                    chqAmount = Convert.ToDecimal(txtTotalGrdPayment.Text);
                }


                txtAmountPay.Text = (cashAmount + chqAmount).ToString();
        ...
        private void ReceiptVoucherTotalCalculation() {
            try
            {
                decimal cashAmount = 0;
                decimal chqAmount = 0;

                if (!txtCashAmount.Text.Equals(""))
                {
                    cashAmount = Convert.ToDecimal(txtCashAmount.Text);
                }

                if (!txtChqTotal.Text.Equals("")) {
                    chqAmount = Convert.ToDecimal(txtChqTotal.Text);
                }
                

                txtAmountReceipt.Text = (cashAmount + chqAmount).ToString();
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "Exception", MessageBoxButtons.OK, MessageBoxIcon.Information); }
        }
        ...
        private void loadReceipt()
        {
            try {
                ...
                dayBook.loadAllReceiptAccounts(cmbReceipt);
        ...
        private void loadPayment()
        {
            try
            {
                ...
                dayBook.loadAllPaymentAccounts(cmbSupplierPay);
                ...
                if (grdPayment.Rows.Count > 0)
                {
                    if (e.RowIndex >= 0)
                    {
                        ...
                        if (diag == DialogResult.Yes)
                        {
                            ...
                            txtPayChqAmount.Text = row.Cells["AMOUNT"].Value.ToString();
        ...
        private void ReceiptsGrid_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            ReceiptsGrid.Columns["REC_PERSON_ID"].Visible = false;
        }
        ...
        private void PaymentGrid_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            PaymentGrid.Columns["PAYMENT_ID"].Visible = false;
     ...
     //               db.dr.Close();
        ...
        {
            ReceiptVoucherTotalCalculation();
            //try
            //{
        ...
        private void cmbReceipt_Leave(object sender, EventArgs e)
        {
            if (daybookID.Equals(""))
            {
                int accountId = cmbReceipt.SelectedValue == null ? 0 : Convert.ToInt32(cmbReceipt.SelectedValue.ToString());
                if (db.GetAccountStatus(accountId).Equals("1"))
                {
        ...
        private void cmbSupplierPay_Leave(object sender, EventArgs e)
        {
            if (paybookID.Equals(""))
            {
                int accountId = cmbSupplierPay.SelectedValue == null ? 0 : Convert.ToInt32(cmbSupplierPay.SelectedValue.ToString());
                if (db.GetAccountStatus(accountId).Equals("1"))
                {
                    MessageBox.Show("Account is Deactivate, Please select another account.", "Error");
                    cmbSupplierPay.Focus();
        ...
        private void cmbAccountGV_Leave(object sender, EventArgs e)
        {
            if (gv_id.Equals(""))
            {
                int accountId = cmbAccountGV.SelectedValue == null ? 0 : Convert.ToInt32(cmbAccountGV.SelectedValue.ToString());
                if (db.GetAccountStatus(accountId).Equals("1"))
                {
                    MessageBox.Show("Account is Deactivate, Please select another account.", "Error");
                    cmbAccountGV.Focus();
        ...
        private void txtChqTotal_KeyPress(object sender, KeyPressEventArgs e)
        {
            ReceiptVoucherTotalCalculation();
        }
        ...
        private void txtPayChqAmount_TextChanged(object sender, EventArgs e)
        {
            PaymentTotalCalculation();
        }
    }
