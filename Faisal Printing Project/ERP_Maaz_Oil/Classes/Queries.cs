...
namespace ERP_Maaz_Oil.Classes
{
    class Queries
    {
        ...
        {
            return @"SELECT A.GV_ID,A.GV_CODE AS [VOUCHER #],CONVERT(varchar(MAX),A.DAATE) AS [DATE],
C.COA_NAME as [ACCOUNT],B.DEBIT as [DEBIT],B.CREDIT as [CREDIT],B.NARRATION AS [NARRATION]
            FROM GENERAL_VOUCHER_M A
            INNER JOIN GENERAL_VOUCHER_D B ON A.GV_ID = B.GV_ID
            INNER JOIN COA C ON B.COA_ID = C.COA_ID
            GROUP BY A.GV_ID,A.DAATE,A.GV_CODE,C.COA_NAME,B.DEBIT,B.CREDIT,B.NARRATION oRDER BY A.DAATE DESC";
        }
        ...
        {
            return @"SELECT '0' AS[ID],'--SELECT DEBIT ACC--' AS[NAME] UNION ALL SELECT COA_ID,COA_NAME FROM COA";
        }
        public static string loadCredit()
        {
            return @"SELECT '0' AS[ID],'--SELECT CREDIT ACC--' AS[NAME] UNION ALL SELECT COA_ID,COA_NAME FROM COA";
        }
        ...
        public static string loadReceipts()
        {
            return @"SELECT D.DAY_BOOK_ID,DATE,ISNULL(D.DEBIT_AC,'-') as [DEBIT_AC],C.COA_NAME AS[DEBIT],D.CREDIT_AC,E.COA_NAME AS[CREDIT],
                    ...
                    ORDER BY DATE DESC,DAY_BOOK_ID DESC";
        }
        ...
        public static string loadPayment()
        {
            ...
            return @"WITH A AS (
                    ...
                    ISNULL((
                        SELECT SUM(C.AMOUNT) 
                        FROM DAY_BOOK A 
                        INNER JOIN DAY_BOOK_CHQ D ON A.DAY_BOOK_ID = D.DAY_BOOK_ID
                        INNER JOIN CHQ C ON D.CHQ_ID = C.CHQ_ID
                        WHERE A.DAY_BOOK_ID = B.DAY_BOOK_ID
                    ),0) as [CHQ_TOTAL],B.AMOUMT as [TOTAL]
                    ,B.PAID_TO,B.DESCRIPTION ,B.ENTRY_OF,B.DEBIT_AC AS [PAYMENT_ID]
                    FROM DAY_BOOK B 
                    WHERE B.ENTRY_OF = 'PAYMENT VOUCHER'
                    )
                    SELECT [DATE],DAY_BOOK_ID,[PAYMENT ACCOUNT],CASH_AMOUNT,CHQ_TOTAL,(CASH_AMOUNT+CHQ_TOTAL) AS [TOTAL],
                    PAID_TO,[DESCRIPTION],ENTRY_OF,PAYMENT_ID
                    FROM A
                    ORDER BY DATE DESC,DAY_BOOK_ID DESC";
        }
