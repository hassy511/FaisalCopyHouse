...
namespace ERP_Maaz_Oil.Classes
{
    class cls_Bank_Payment
    {
        ...
        //load cvr grid
        public void load_bvp_grid(DataGridView dg)
        {
            query = @"SELECT A.BPV_ID,A.BPV_CODE AS [VOUCHER #],A.TRANS_CODE,A.DAATE AS [DATE],A.BANK_ID,C.COA_NAME AS [BANK],A.COA_ID,B.COA_NAME AS [ACCOUNT],LTRIM(STR(a.AMOUNT,25,2)) as [AMOUNT],A.CHQ_NO AS [CHEQUE #],CONVERT(varchar(MAX),A.CHQ_DAATE) AS [CHEQUE DATE], A.ACCOUNT_OF AS [DESCRIPTION],A.PAID_TO AS [PAID TO]
                    FROM BANK_PAY_VOUCHER A,COA B,COA C
        ...
        public void bvp_grid_search_bankAccount(string keyword, DataGridView grdSEARCH)
        {
            (grdSEARCH.DataSource as DataTable).DefaultView.RowFilter = string.Format(@"
        ...
        public int save_bpv_details(string save_query)
        {
        ...
        {
            query = "select count(BPV_ID)+1 from BANK_PAY_VOUCHER";
            lbl.Text = "BPV-"+cls_fhp.generate_voucher_code(query)+"-"+DateTime.Now.Year;
        }
        ...
        public int insert_ledger_entry(string date, int debit_coa_id, int credit_coa_id, string reference_id, string entry_of, float amount, string description,string code)
        {
            string query = @"IF EXISTS (SELECT REF_ID FROM LEDGERS WHERE REF_ID = '" + reference_id + @"')
                ...
                DELETE FROM LEDGERS WHERE REF_ID = '" + reference_id + "' AND ENTRY_OF = '" + entry_of + @"'
                INSERT INTO LEDGERS VALUES('" + date + "', '" + debit_coa_id + @"',
                (SELECT MAX(BPV_ID) FROM BANK_PAY_VOUCHER),'" + entry_of + @"','" + code + @"',
                '" + amount + "','0','" + description + "','" + Classes.Helper.userId + @"',
                '" + DateTime.Now + @"',NULL,NULL,1,NULL);
                INSERT INTO LEDGERS VALUES('" + date + "', '" + credit_coa_id + @"',
                (SELECT MAX(BPV_ID) FROM BANK_PAY_VOUCHER),'" + entry_of + @"','" + code + @"','0',
                '" + amount + "','" + description + "','" + Classes.Helper.userId + @"',
                '" + DateTime.Now + @"',NULL,NULL,1,NULL);
                END";
